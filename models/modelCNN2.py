...
class Model(nn.Module):
    def __init__(self, conf):
        ...
        self.cnn1 = nn.Conv1d(1, conf.filters, conf.kernelSize)
        self.cnn2 = nn.Conv1d(conf.filters, conf.filters, conf.kernelSize)
        self.cnn3 = nn.Conv1d(conf.filters, conf.filters, conf.kernelSize)

        outSize = conf.dimX - 3*(conf.kernelSize -1)

        if conf.dropout > 0:
            self.dropout = nn.Dropout(p=conf.dropout)

        self.fc1 = nn.Linear(outSize*conf.filters, outSize*conf.filters)
        self.fc2 = nn.Linear(outSize*conf.filters, 4)
