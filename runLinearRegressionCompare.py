...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        ...
        lossValue = loss(pred['test'], y['test'])
        # lossValue = loss(pred['valid'], y['valid'])

        print("Test sloss: {}".format(lossValue))

        t1,coh,coh_err = np.loadtxt(conf.compareDataset)

        predReal = reg.predict(coh.reshape(1,-1))
        lossReal = loss(predReal, np.array(conf.compareParameters).reshape(1,-1))
        print("Compare with {}:\nSloss: {},\nReal: {}\nPred: {}".format(conf.compareDataset, lossReal, conf.compareParameters, predReal[0]))
